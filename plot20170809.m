...
data(1:site,3:7) = sqrt(data(1:site,3:7))*8;

for i = 1:site
    [r,order] = sort(data(i,3:7),'descend');
    for j = 1:5
        if (r(j) > 0)
            geoshow(data(i,1),data(i,2), 'DisplayType','Point','Marker','.','MarkerEdgeColor',c(order(j),:),'Markersize',r(j));
        end
    end
end
